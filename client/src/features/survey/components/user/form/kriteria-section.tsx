...
export function KriteriaSection({ kriteria, onChange }: KriteriaSectionProps) {
  ...
  useEffect(() => {
    ...
    } else if (typeof ageMin === 'number') {
      usia = Array.from({ length: 101 - ageMin }, (_, i) => ageMin + i);
    } else if (typeof ageMax === 'number') {
      usia = Array.from({ length: ageMax }, (_, i) => i + 1);
    }
  ...
  const handleAgeInput = (
    value: string,
    setter: (n: number | undefined) => void
  ) => {
  ...
  return (
    <fieldset className="grid md:grid-cols-2 grid-cols-1 gap-5">
      <FormGroup label="Region Domisili" htmlFor="region-input">
        <div className="flex gap-2">
          <Input
            ...
            className="w-48 bg-none placeholder:text-accent-1/50"
      ...
      <FormGroup label="Umur (rentang usia)" htmlFor="umur-min">
        <div className="flex gap-4 items-center">
          <Input
            ...
            type="number"
            placeholder="Min"
            className="w-20 placeholder:text-accent-1/50"
            min={1}
            max={100}
            value={ageMin ?? ''}
      ...
      <FormGroup label="Jenis Kelamin" htmlFor="jenis_kelamin">
        <select
          id="jenis_kelamin"
          className="border bg-none rounded px-2 py-1"
          value={jenis_kelamin ?? ''}
