...
export function KriteriaSection({ kriteria, onChange, disabled = false }: KriteriaSectionProps) {
  ...
  useEffect(() => {
    ...
    onChange({ region, status, usia, jenis_kelamin });
    ...
    <fieldset className="grid md:grid-cols-2 grid-cols-1 gap-5">
      <FormGroup label="Region Domisili" htmlFor="region-input">
        <div className="flex gap-2">
          ...
          <Button
            ...
            onClick={() => handleAdd(regionInput, setRegion, region, () => setRegionInput(''))}
            disabled={disabled}
          >
            <Plus />
          </Button>
        ...
        <div className="flex flex-wrap gap-2 mt-2">
          {region.map((item, index) => (
            <span key={index} className="border rounded px-2 py-1 sm:text-sm text-xs flex items-center">
              {item}
              {!disabled && (
                <button type="button" className="ml-2 text-red-500" onClick={() => handleRemove(index, setRegion, region)}>
                  <X />
                </button>
              )}
            </span>
      ...
      <FormGroup label="Status Latar Belakang" htmlFor="status-input">
        <div className="flex gap-2">
          ...
          <Button
            ...
            variant="outline"
            className="text-[#232323] hover:opacity-80"
            onClick={() => handleAdd(statusInput, setStatus, status, () => setStatusInput(''))}
            disabled={disabled}
          >
            <Plus />
          </Button>
        ...
        <div className="flex flex-wrap gap-2 mt-2">
          {status.map((item, index) => (
            <span key={index} className="border rounded px-2 py-1 sm:text-sm text-xs flex items-center">
              {item}
              {!disabled && (
                <button type="button" className="ml-2 text-red-500" onClick={() => handleRemove(index, setStatus, status)}>
                  <X />
                </button>
              )}
            </span>
      ...
      <FormGroup label="Jenis Kelamin" htmlFor="jenis_kelamin">
        <select
          ...
          disabled={disabled}
        >
          <option className="text-[#232323]" value="">Semua</option>
          <option className="text-[#232323]" value="laki-laki">Laki-laki</option>
          <option className="text-[#232323]" value="perempuan">Perempuan</option>
        </select>
      </FormGroup>
    </fieldset>
  );
